...
function computeFinalScores(items: AppraisalItemInput[]) {
    ...
    const weightedAverage = items.reduce((sum, item) => sum + item.points * item.weight, 0) / totalWeight;
...
async function getAppraisalOr404(appraisalId: string) {
    return prisma.appraisal.findUnique({
        ...
        include: {
            cycle: true,
            user: {
                select: { id: true, email: true, firstName: true, lastName: true, departmentId: true }
            },
            items: true,
...
async function canViewAppraisal(appraisal: NonNullable<Awaited<ReturnType<typeof getAppraisalOr404>>>, userId: string, roles: string[]) {
    if (appraisal.userId === userId || roles.includes('HR') || roles.includes('SUPER_ADMIN')) {
        return true;
    }
    ...
    try {
        const appraisals = await prisma.appraisal.findMany({
            include: {
                ...
                user: {
                    select: { id: true, email: true, firstName: true, lastName: true, department: { select: { name: true } } }
                }
            },
...
router.get('/hod/review-list', authenticateRequest, requireRoles('HOD', 'HR', 'SUPER_ADMIN'), async (req: AuthenticatedRequest, res, next) => {
    try {
...
router.get('/committee/review-list', authenticateRequest, requireRoles('COMMITTEE', 'HR', 'SUPER_ADMIN'), async (req: AuthenticatedRequest, res, next) => {
    try {
...
router.put('/:appraisalId', authenticateRequest, async (req: AuthenticatedRequest, res, next) => {
    try {
        ...
        if (appraisal.userId !== userId) {
            return res.status(403).json({ success: false, message: 'Can only edit your own appraisal' });
        }

        if (appraisal.status !== 'DRAFT' || appraisal.locked) {
            return res.status(400).json({ success: false, message: 'Only unlocked draft appraisals can be edited' });
        }

        const existingItemIds = parsed.items.filter(item => item.id).map(item => item.id as string);
        ...
        if (existingItemIds.length > 0) {
            ...
            if (ownedItems.length !== existingItemIds.length) {
                return res.status(400).json({ success: false, message: 'One or more appraisal items are invalid' });
            }
        }
...
router.post('/:appraisalId/committee-review', authenticateRequest, requireRoles('COMMITTEE', 'HR', 'SUPER_ADMIN'), async (req: AuthenticatedRequest, res, next) => {
    try {
        const { appraisalId } = req.params;
        const userId = req.auth?.sub;
        ...
        const isAssignedCommitteeMember = appraisal.committeeAssignments.some(assignment =>
            assignment.committee.members.some(member => member.id === userId)
        );
        ...
        if (!['HOD_REVIEW', 'COMMITTEE_REVIEW'].includes(appraisal.status)) {
            return res.status(400).json({ success: false, message: 'Appraisal is not ready for committee review' });
        }
...
// Submit appraisal
router.post('/:appraisalId/submit', authenticateRequest, async (req: AuthenticatedRequest, res, next) => {
    try {
        const { appraisalId } = req.params;
        const userId = req.auth?.sub;
        ...
        if (appraisal.userId !== userId) {
            return res.status(403).json({ success: false, message: 'Can only submit your own appraisal' });
        }
        ...
        if (appraisal.items.length === 0) {
            return res.status(400).json({ success: false, message: 'Appraisal must contain at least one item before submission' });
        }
